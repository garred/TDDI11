...
//		isAcOn &=  0x00000001;
		...
		temperatureDesired = 14.5+temperatureDesired_tmp*0.5 ; // Assignment
		...
		// Assignment
		if (temperatureMeasured_decimal == 31) {
			if (temperatureMeasured_fractional == 0)
				temperatureMeasured = -1;
			else
				temperatureMeasured = 31;
		} else 	
			temperatureMeasured = temperatureMeasured_decimal + temperatureMeasured_fractional*0.2; // Assignment
		...
		// Assignment	
		// Assignment	
		else if (isHeaterOn && (temperatureDesired <= temperatureMeasured))
		{
			isCoolerOn = false;
			isHeaterOn = false;
		}
		...
		if (isCoolerOn)
			outputRegister |= (0x00000001 << 4); // Assignment	
		...
		if (isHeaterOn)
			outputRegister |= (0x00000001 << 5); // Assignment	
...
/*
    printf("Input register:  ");
		print_bin(inputRegister);
		printf("\nOutput register: ");
		print_bin(outputRegister);
		printf("\n");
		*/
